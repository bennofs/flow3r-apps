...
class SIPMessage:
    def __init__(self, data: bytes):
        self.SIPCompatibleVersions = inits.SIPCompatibleVersions
        self.SIPCompatibleMethods = inits.SIPCompatibleMethods
        self.heading = b""
                ...
                self.body["m"].append(
                    {
                        ...
                        "port": int(port),
                        "port_count": count,
                        "protocol": inits.RTP.RTPProtocol(d[2]),
                        "methods": methods,
                        "attributes": {},
                    }
                )
...
class SIPClient:
    ...
    def gen_sip_version_not_supported(self, request: SIPMessage) -> str:
        ...
        response += f"User-Agent: pyVoIP {inits.__version__}\r\n"
        response += 'Warning: 399 GS "Unable to accept call"\r\n'
        response += f"Allow: {(', '.join(inits.SIPCompatibleMethods))}\r\n"
    ...
    def gen_first_response(self, deregister=False) -> str:
        ...
        regRequest += f'Allow: {(", ".join(inits.SIPCompatibleMethods))}\r\n'
        regRequest += "Max-Forwards: 70\r\n"
        regRequest += "Allow-Events: org.3gpp.nwinitdereg\r\n"
        regRequest += f"User-Agent: pyVoIP {inits.__version__}\r\n"
    ...
    def gen_subscribe(self, response: SIPMessage) -> str:
        ...
        subRequest += f"User-Agent: pyVoIP {inits.__version__}\r\n"
    ...
    def gen_ok(self, request: SIPMessage) -> str:
        ...
        okResponse += f"User-Agent: pyVoIP {inits.__version__}\r\n"
        okResponse += f"Allow: {(', '.join(inits.SIPCompatibleMethods))}\r\n"
    ...
    def gen_answer(
        ...
        body += f"s=pyVoIP {inits.__version__}\r\n"
        # TODO: Check IPv4/IPv6
    ...
    def gen_invite(
        ...
        invRequest += f"Allow: {(', '.join(inits.SIPCompatibleMethods))}\r\n"
        invRequest += "Content-Type: application/sdp\r\n"
        invRequest += f"User-Agent: pyVoIP {inits.__version__}\r\n"
    ...
    def gen_bye(self, request: SIPMessage) -> str:
        ...
        byeRequest += f"User-Agent: pyVoIP {inits.__version__}\r\n"
        byeRequest += f"Allow: {(', '.join(inits.SIPCompatibleMethods))}\r\n"
    ...
    def gen_ack(self, request: SIPMessage) -> str:
        ...
        ackMessage += f"User-Agent: pyVoIP {inits.__version__}\r\n"
